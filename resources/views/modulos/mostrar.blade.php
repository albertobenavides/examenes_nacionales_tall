...
<script>
    $(function(){
        ...
        $('.mostrarVideo').click(function(){
            @if(Auth::user()->rol_id == 1 or (Auth::user()->pagos->where('curso_id', $modulo->curso->id)->where('fin', '>=', Carbon\Carbon::today())->count() > 0 and Auth::user()->pagos->where('curso_id', $modulo->curso->id)->where('fin', '>=', Carbon\Carbon::today())->sortByDesc('promo_id')->first()->promo->videos == true))
        ...
        $('.mostrarPDF').click(function(){
            @if(Auth::user()->rol_id == 1 or (Auth::user()->pagos->where('curso_id', $modulo->curso->id)->where('fin', '>=', Carbon\Carbon::today())->count() > 0))
            $('.modal-title').html($(this).attr('data-nombre'));
            $('#preguntasTema').hide();
            $('#videoTema').parent().hide();
            $('#videoYoutube').hide();
        ...
        $('.mostrarPreguntas').click(function(){
            $('#ayuda').html('');
            @if(Auth::user()->rol_id == 1 or (Auth::user()->pagos->where('curso_id', $modulo->curso->id)->where('fin', '>=', Carbon\Carbon::today())->count() > 0 and Auth::user()->pagos->where('curso_id', $modulo->curso->id)->where('fin', '>=', Carbon\Carbon::today())->sortByDesc('promo_id')->first()->promo->examenes == true))
                            ...
                            <tr class="bg-white">
                                ...
                                <td class="text-center">
                                    @if($t->preguntas->count() > 0)
                                        <a href="#" data-toggle="modal" data-nombre="{{$t->nombre}}" data-target="#mostrarVideo" class="mostrarPreguntas button" tema_id="{{$t->id}}"><i class="fas fa-tasks fa-2x"></i></a>
                                    @endif
                                </td>
                                <td class="text-center">
                                    @if($t->preguntar > 0)
